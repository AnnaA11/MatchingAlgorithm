...
public:
	...
	//Gets the position of vertex i in the tree
	//0 for i not in T, 1 for i in Even(T), 2 for i in Odd(T)
	int get_position_in_tree(int i);
	
	//Changes the position of vetrex i to even 
	void set_position_even(int i);
	
	//Changes the position of vetrex i to odd
...
UnionFind::UnionFind(std::size_t size)
	...
	_size(size)
	{
		for (unsigned int i = 0; i<size; i++){
			...
			//apex of pseudonode
			//warning: position and apex are only up to date for the root of a pseudonode
