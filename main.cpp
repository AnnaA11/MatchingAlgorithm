...
bool find_M_agumenting_path_inclass(ED::Graph const & Tree, ED::Graph const & M, std::vector <bool> & visited, UF::UnionFind const & Partition, std::vector <ED::NodeId> & p, ED::NodeId start, ED::NodeId end) {
...
std::vector <ED::NodeId> find_M_agumenting_path(ED::Graph const & M, ED::Graph const & Tree, UF::UnionFind const & Partition, std::vector <ED::NodeId> const & p) {
	int start=0;
	int end=0;
...
ED::NodeId check_neighbors_of_node(ED::NodeId v, ED::Graph const & G, std::vector <int> const & Partition) {
	auto const & node = G.node(v);
	for (auto const & neighbor_id : node.neighbors()) {
		if(Partition.position_in_tree(neighbor_id)!=2 && Partition.equals(neighbor_id, v)==0 {
			return neighbor_id;
		}
	}
	return v;
...
void update_partition_edges(ED::Graph G, std::vector <ED::NodeId> path, UF::UnionFind & Partition,std::vector <std::pair<ED::NodeId, ED::NodeId>> Edges){
	...
	Partition.change_position_even(k);
}
...
int build_up_tree(ED::Graph const & G, ED::Graph & M) {
	...
	//build up tree with this root
		...
		for (auto const & neighbor_id : node.neighbors()){
			Edges.push_back((root, neighbor_id));
		}
		while(Edges.size()>0 && i==0) {
			...
			if(Partition.position_in_tree(end)!=2) i=grow_tree(G,M,root,Tree,Partition,start,end,Edges); 
		}
		if(i==1){
...
{
	ED::NodeId node;
	if(Partition.position_in_tree(end)==0 && check_node_M_exposed(end,M)){
		Tree.add_edge(end,start);
		Partition.change_position_odd(start);
	...
	else if(Partition.position_in_tree(end)==0){
		//update Tree and Positions in Tree
		Tree.add_edge(start,end);  
		Tree.add_edge(end,(M.node(end)).neighbors().at(0));
		Partition.change_position_even((M.node(end)).neighbors().at(0));
		Partition.change_position_odd(end);
				
		//update "Edges" by adding all edges incident to the vertex  which is mached to "end" 
		auto const & node = G.node(end);
		for (auto const & neighbor_id : node.neighbors()) {
			if(neighbor_id!=start) Edges.push_back((end,neighbor_id));
		}
...
//ToDo: delete
/*
int build_up_tree(ED::Graph const & G, ED::Graph & M, ED::NodeId const start) {
	ED::Graph Tree{G.num_nodes()};
